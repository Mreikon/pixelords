...
class Object(pygame.sprite.Sprite): # Parent class for all objects
	...
	def collision(self, map): # Detect collisions
		...
		xrange = []
		if self.dx > 0:
			xrange = range(int(self.x), int(self.x+self.dx+1.414213562373095*self.dx/math.sqrt(self.dx**2+self.dy**2+0.01)+1), 1)
		elif self.dx < 0:
		...
		yrange = []
		if self.dy > 0:
			yrange = range(int(self.y), int(self.y+self.dy+1.414213562373095*self.dy/math.sqrt(self.dx**2+self.dy**2+0.01)), step)
		elif self.dy < 0:
